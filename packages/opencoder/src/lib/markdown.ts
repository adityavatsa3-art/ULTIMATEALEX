...
// workaround for marked to support links in text
  ...
  switch (token.type) {
    ...
    case "code":
      if (token.lang && supportsLanguage(token.lang)) {
        return highlight(token.text, { language: token.lang, theme: "github-dark" }) + EOL
      } else {
        logError(
          `Language not supported while highlighting code, falling back to markdown: ${token.lang}`,
        )
        return highlight(token.text, { language: "markdown", theme: "github-dark" }) + EOL
      }
    case "codespan":
      // inline code
      return chalk.bgGray.red(token.text)
    case "em":
    ...
    case "heading":
      switch (token.depth) {
        case 1: // h1
          return (
            chalk.bold.blue("#" + (token.tokens ?? []).map((_) => format(_)).join("")) + EOL + EOL
          )
        case 2: // h2
          return (
            chalk.bold.blue("##" + (token.tokens ?? []).map((_) => format(_)).join("")) + EOL + EOL
        ...
        default: // h3+
          return (
            chalk.blue.bold(
              "#".repeat(token.depth) + " " + (token.tokens ?? []).map((_) => format(_)).join(""),
            ) +
            EOL +
            EOL
          )
    ...
    case "list": {
      return token.items
        .map((_: Token, index: number) =>
          format(_, listDepth, token.ordered ? token.start + index : null, token),
        )
        .join("")
    }
    case "list_item":
