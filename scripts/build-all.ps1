...
Write-Host "`nBuilding Omni-LLM-Suite (all packages)...`n" -ForegroundColor Magenta
...
pnpm run build
Pop-Location
Write-Host "   [OK] TypeScript build complete" -ForegroundColor Green
...
if (!$SkipDotnet -and (Get-Command dotnet -ErrorAction SilentlyContinue)) {
    ...
    Pop-Location
    Write-Host "   [OK] .NET Gateway built" -ForegroundColor Green
}
...
if (!$SkipRust -and (Get-Command cargo -ErrorAction SilentlyContinue) -and (Test-Path "$root\packages\rtk\Cargo.toml")) {
    ...
    if (Test-Path "target\release\rtk.exe") {
        ...
        Write-Host "   [OK] rtk.exe -> bin/rtk.exe" -ForegroundColor Green
    }
    Pop-Location
}

Write-Host "`n[OK] All builds complete!`n" -ForegroundColor Green
